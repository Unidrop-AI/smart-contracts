...
abstract contract TwoPhaseOwnable is Context {
    ...
    modifier onlyOwner() {
        require(
            owner() == _msgSender(),
            "TwoPhaseOwnable: caller is not the owner"
        );
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(
            newOwner != address(0),
            "TwoPhaseOwnable: new owner is the zero address"
        );
    ...
    function acceptOwnership() public virtual {
        require(
            ...
            "TwoPhaseOwnable: sender is not the next choosen one!"
        );
...
contract MetaStrike is ERC20, ERC20Burnable, Pausable, TwoPhaseOwnable {
    ...
    function checkBlacklisted(address _user) external view returns (bool) {
    ...
    function batchBlackList(
        ...
        if (_blacks.length == 1) {
            ...
            require(
                _evils.length == _blacks.length,
                "MTS: Input Format Mismatch!"
            );
            for (uint256 i = 0; i < _evils.length; i++) {
